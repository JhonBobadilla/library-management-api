...
```bash
MSYS_NO_PATHCONV=1 docker cp database/backup/library_test_data.dump library-db:/tmp/library_test_data.dump
MSYS_NO_PATHCONV=1 docker exec library-db pg_restore -U library_user -d library_db --clean --if-exists /tmp/library_test_data.dump
```

> **Nota:** `MSYS_NO_PATHCONV=1` se usa en Git Bash para evitar la conversión automática de rutas.
