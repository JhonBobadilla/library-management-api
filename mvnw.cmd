...
@REM ----------------------------------------------------------------------------
@RM Maven Start Up Batch script
@REM
...
:OkJHome
if exist "%JAVA_HOME%\bin\java.exe" goto checkJHome
echo.
...
:checkJHome
if exist "%JAVA_HOME%\bin\javaw.exe" goto init
echo.
echo Error: JAVA_HOME is set to an invalid directory. >&2
echo JAVA_HOME = "%JAVA_HOME%" >&2
echo Please set the JAVA_HOME variable in your environment to match the >&2
...
@REM Workaround for https://github.com/spring-projects/spring-boot/issues/13629
if not "%MVNW_REPOURL%" == "" (
...
echo.
echo Error: cannot find maven-wrapper.jar. >&2
echo Please download it from %DOWNLOAD_URL% >&2
echo. >&2
goto error

:run
@REM Execute Maven
"%JAVA_HOME%\bin\java.exe" %JVM_CONFIG_MAVEN_PROPS% %MAVEN_OPTS% %MAVEN_DEBUG_OPTS% -classpath %WRAPPER_JAR% "-Dmaven.multiModuleProjectDirectory=%MAVEN_PROJECTBASEDIR%" %WRAPPER_LAUNCHER% %*
